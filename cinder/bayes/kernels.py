"""
kernels.py — closed-form conjugate kernels + likelihood-spec executor for PTV UCs.

CINDER vendoring note. This module is vendored verbatim from the
2ndOpinionMD-MVP repository at the locked commit 00eaa9eb (per
IMPLEMENTATION_PLAN.md "Decisions locked"). The only edits made during
vendoring were:

  - File renamed bayes.py -> kernels.py to avoid shadowing the
    cinder.bayes package name.
  - Top-level import "from server.eoh.uc import ..." rewritten to
    relative "from .uc import ...".
  - This vendoring header added.

No logic, dataclass, default prior, or default likelihood-spec rule was
modified. See cinder/bayes/PROVENANCE.md for the full vendoring contract,
re-vendoring procedure, and the rationale for closed-form kernels in the
per-patient detection layer.
...
Three conjugate families, three returned UCs (per ``cinder/bayes/uc.py``):
...
from .uc import UncertaintyCarrier, canonical_spec_hash, confidence_from_band
